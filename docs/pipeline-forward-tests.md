...
適用開始: 本要件は本族を導入した改訂（PR #24）の次の改訂から適用する。PR #24 が導入した収束機構（予算ゲート・有限 closure・HANDOFF）自体の初回検証は、owner の判断により forward test ではなく実運用計測（issue #23 の受け入れ条件「同規模 PR で 2 時間以内」）で行う。
