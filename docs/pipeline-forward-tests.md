...
シナリオ族 1・2 は must-fix が少ない軽負荷経路しか通らないため、レビューループの収束性は本族で検証する（背景: issue #23）。skill のレビュー・ゲート・予算まわりを改訂する PR は、本族の実行を合格条件に含める。owner の明示判断がある場合に限り、実行を実運用計測（cycle 表による回復確認）で代替できる。代替の決定と根拠は該当 PR に記録する。
