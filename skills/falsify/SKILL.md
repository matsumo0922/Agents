...
3. スコープ判定の妥当性（1 実装者 / 1 レビュアーで網羅可能か。受け入れ条件に紐付かない拡張が混入していないか。拡張混入の判定は最小充足の ladder — 不要なら書かない / 既存の再利用 / stdlib / platform native / 導入済み依存 / 1 行 / 最小実装 — を基準とし、ladder のより上の段で足りる設計要素を反例として挙げる）
