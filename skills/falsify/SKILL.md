...
- モデルの既定: Claude subagent を使える環境では Opus 5（effort はセッション既定を継承）、GPT を使える環境では gpt-5.6-sol / high（`gpt-high` agent）。反証対象が発動判定の高リスク基準に触れる場合、呼び出し側が spawn 前に 1 段上（Fable 5、または gpt-5.6-sol / xhigh）へ昇格する。issue-pr-autopilot から呼ばれる場合の環境別の割当表は同スキルの「モデル割当」を正とする
...
3. スコープ判定の妥当性（1 実装者 / 1 レビュアーで網羅可能か。受け入れ条件に紐付かない拡張が混入していないか。拡張混入の判定は ponytail スキルの ladder を基準とし、ladder のより上の段で足りる設計要素を反例として挙げる。呼び出し側は falsifier の brief に ladder を展開して渡し、ponytail スキルが読めない環境ではこの基準を不適用とする）
4. 高リスク未検証前提の見落とし（「事実」とされた値に確認方法が無いものはないか。暗黙の前提はないか）
5. レビュー可能性と実装可能性（人間が 1 本の PR としてレビューできる規模か。複数の依存 PR で配送する場合は各 PR がその規模に収まるか。実装者が設計とリポジトリだけで完遂できるか）
...
3. **stage-out**：対策を後続の実装単位へ分離し、該当機能を default-off / feature flag で隔離する。分離先は 2 種を区別する。同一 intent 内なら同じ change の後続の実装単位（複数 PR で配送する場合は上段の実装 PR、単一 PR なら後続 PR）、独立して archive できる別 intent なら別 change（子 issue）とする。いずれも隔離により反例の前提が崩れることを falsifier が確認する
...
3. 検証不能で自走中なら停止せず、fail-safe 側（既存状態を変更しない側）を仮決めし、（高リスク・要人間確認）マークを付けて進行する。実装方式、不可逆 migration、安全性、permission 境界を左右する仮決めは、採らなかった選択肢と分岐条件も併記する。reversible かつ additive な実装単位を切り出せるなら、仮決めの影響範囲をその単位に閉じる
