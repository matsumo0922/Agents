...
- 反例: <ベクトル番号 / blocking・non-blocking・stage-out 提案のいずれか / 反例の内容（入力・状態 → 誤った結果）/ 根拠となるコード箇所> を列挙。なければ「各ベクトルで探索した結果反例なし」
...
**スコープ軸**: 反例が突く欠陥が今回の変更で導入・悪化するものではなく、issue の受け入れ条件・変更対象の外に既存で存在する場合は、blocking ではなく **stage-out 提案**（後続 stage / 別 issue での対処）として返す。falsification_result にはその区分を明記する。
...
- すべての処置を設計の `### 反証` セクションへ、反例の blocking / non-blocking / stage-out 区分とともに記録する。
