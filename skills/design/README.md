...
- **独立反証**: 変更が客観条件リスト（cross-layer / safety / migration / security / 複数 consumer / DB hot path）に該当する場合、設計確定前に clean context の falsifier サブエージェントが `references/falsifier-rubric.md` の反証 5 ベクトルで設計を攻撃する。該当判定は main agent が行い、architect の自己申告に依存しない。該当しない設計は architect が自己反証する
- **スコープ判定**: 「1 worker / 1 reviewer で網羅できるか」を客観 signal を根拠に判定し、網羅不能なら staged PR 分割案（各 stage が単独で merge・deploy 可能かつ backward-compatible）を設計成果物にする
