...
- 回答を台帳に追記し、同じ architect を次の形式で継続する（再 spawn しない。継続できない環境では前回の要約と回答を添えて再依頼する）。

```text
<followup_request>
- ユーザーの回答: <質問ごとの選択と自由記述>
- 追加の確認事項: <反論・質問のうちコード読解が必要なもの。なければ「なし」>
- タスク: 回答と確認事項を設計へ反映し、台帳を更新する。反映で新たに要質問の論点が生じたら返し、なければ確定版を /tmp/design/<slug>.design.md へ書き出して FINAL と返す。
</followup_request>
```
...
architect が確定版を `/tmp/design/<slug>.design.md` へ書き出したら（要質問の論点が無く質問ラウンドを行わなかった場合は、書き出しを直接依頼する）、main agent はこれを読み、要件・質問ラウンドの決定との整合を確認してからユーザーに報告する（以降のユーザーとの設計議論はこのドキュメントを根拠に main agent が直接応答し、コードレベルの再調査が必要なときだけ architect を継続する）。中身は次の形式とする。issue-pr-autopilot が回収する「## 設計」セクションと同一形式のため、issue へ投稿するときは本文へそのまま追記できる。
...
- architect = `gpt-5.6-sol`（`model_reasoning_effort = "xhigh"`）。設計は 1〜2 パスで品質が決まり、高 effort の品質向上が最も効く役割のため消費増を許容する。読み込み中心のため組み込みの explorer エージェントが向く。
