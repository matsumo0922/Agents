...
- main agent は薄いオーケストレーターに徹する。コード読解と設計立案は architect サブエージェントに委任し、main agent は対象特定・質問の提示・決定の記録・issue への投稿だけを行う。対象コードや設計の全文を main agent のコンテキストに読み込まない。
...
- 要件そのものが未確定・怪しい場合は、設計に入る前に dig の実行を勧める。dig は前提への挑戦、本スキルは構造の決定を担い、順序は dig → design → 実装とする。
...
質問の反映が終わったら、architect に確定版を `/tmp/design/<slug>.design.md` へ書き出させる（main agent は設計全文をコンテキストに読み込まない）。中身は次の形式とする。issue-pr-autopilot が回収する「## 設計」セクションと同一形式のため、issue へ投稿するときは本文へそのまま追記できる。
