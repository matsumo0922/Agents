...
本スキルは dig / design / issue-pr-autopilot / claude-rescue からなるパイプライン bundle の一部として配布される（`make link` で一括リンク。単体配布はサポートしない）。設計の形式は設計契約が正本であり、issue-pr-autopilot の設計ゲート・レビューも同じ契約を参照する。claude-rescue は Claude サブエージェントを持たない環境で falsifier を呼び出すブリッジとして使う（環境別ヒント参照）。
...
6. 発見事項: 調査中に見つけた、受け入れ条件に紐付かない欠陥・改善点の列挙（各 1 行）。設計には含めない。なければ「なし」
7. ドラフト（/tmp/design/<slug>.draft.md）への書き込み完了の確認
</deliverables>
</architect_instruction>
```
...
rubric の職掌 5 点（反例列挙 / matrix 発動判定の妥当性 / スコープ判定の妥当性 / 見落とし前提 / reviewability・implementation feasibility）を検証し、falsification_result 形式で返してください。処置の決定はしないでください。
</falsifier_instruction>
```
...
コア原則: 細部への目配りは設計では資産になるため、architect は細部まで目の届くモデルの高 effort を使う。falsifier は反例の検出力でなく blocking の較正が核心のため、architect と別ベンダーの判断特化モデルを使う（役割別割当の正本は issue-pr-autopilot の環境別ヒント。effort 原則も同じ: 全役割 high 既定、深さが足りなければ effort でなくモデルを上げる）。コードの長文脈読解を伴うため最軽量ティアを使わない。現行世代のモデルは手順の細目列挙より「ゴール + 制約 + 完了条件」で最も性能が出るため、指示テンプレートの必要部分だけを埋めて渡す。spawn 時のモデル名（effort を指定できる環境では effort も）の申告を忘れない。

### Claude Code

- architect = Opus 4.8、falsifier = Opus 4.8（`model` パラメータで指定）。falsifier は safety / security / 不可逆 migration が主対象の設計に限り Fable 5 へ上げてよい。effort は per-spawn で指定できず session 既定を継承する（役割別に固定したい場合は `.claude/agents/` の role 別エージェント定義で指定する）。質問反映の継続は SendMessage で行い、再 spawn しない。falsifier は新規 spawn する（clean context のため継続にしない）。
...
- architect = `gpt-5.6-sol`（`model_reasoning_effort = "high"`。質問・修正の往復が入る役割のため）。読み込み中心のため組み込みの explorer エージェントが向く。
- **falsifier = Claude Opus 4.8 / high を claude-rescue skill の `claude-bridge.sh` で呼び出す**（`../claude-rescue/scripts/claude-bridge.sh` を絶対パスへ解決して使う）。呼び出しは常に `require_escalated` で起動し、instruction に falsification_result の出力契約を書いて `--expect falsification_result` を付ける。修正後の解消確認は初回 stderr の session_id を `--resume` に渡して同一 falsifier が行う。ブリッジ不通（exit 3）時の fallback は `gpt-5.5 / high`。safety / security / 不可逆 migration が主対象の設計は `--model claude-fable-5` へ上げてよい。
