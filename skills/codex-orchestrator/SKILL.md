...
- 推奨割当は main agent = `gpt-5.6-sol` / `medium`、worker = `gpt-5.6-terra` / `high`、reviewer = `gpt-5.6-sol` / `xhigh`。reviewer の再レビューは同じ thread と effort を維持し、前回指摘と新規 diff だけに入力を絞る
- `max` / `ultra` は使わない。`max` は `xhigh` に対する消費増が大きく、`ultra` はモデル内部で subagent を spawn し、本 skill の並列 worker 構成と二重になる。`gpt-5.6-luna` は primary role に割り当てない
