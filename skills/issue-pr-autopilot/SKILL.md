...
- **意図アンカー**: すべての生成物（設計決定・レビュー指摘・修正 diff）は「issue の受け入れ条件・設計 invariant のどれを守るためか」の紐付けを持つ。紐付けられない指摘は must-fix にできず、紐付けられない変更は diff に含めない（follow-up issue 提案に回す）。
...
- **各指摘に意図アンカーを付ける**: 「issue の受け入れ条件・設計 invariant のどれを守るための指摘か」の紐付け。紐付けられない指摘は内容の正しさに関わらず must-fix にできない: follow-up 提案として分類して報告する（報告自体は禁止しない。分類を強制する）。
