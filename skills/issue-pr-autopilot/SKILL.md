...
- 内部レビューは 1 cycle につき最大 2 round を既定とする。round 2 終了時点で未解消の指摘は修正を続けず、「人間レビューへの引き継ぎ」として PR に記録して確定に進む（例外は予算ゲート参照）。権限や外部依存で進めない場合、仕様判断が必要な場合も未解決事項として報告する。

### 予算ゲート（収束の強制）

次のいずれかが成立したら、main agent はその場で修正ループを止めて分岐する。修正を積み増して解消を目指さない。

1. 同一指摘 cluster が 2 回 PARTIAL 判定になった（incomplete closure ×2）
2. round 1 のレビュー返却から 120 分が経過した
3. PR の diff が round 1 レビュー時点の 2 倍を超えた
4. 指摘への対応に新しい layer の追加・新規サブシステムの設計が必要になった

分岐: 対話中はユーザーに「続行 / 残指摘を人間レビューへ引き継いで確定 / stage 分割」を質問する。自走中の既定は**引き継いで確定**: 未解消の指摘を severity・閉じる条件付きで PR description の「人間レビューへの引き継ぎ」へ記録し、最終コメントを残指摘付きで投稿する。ただし security・データ破壊級の must-fix を未修正のまま確定はできない: 該当機能を default-off / feature flag / stage-out で PR から隔離してから確定し、隔離もできない場合のみ停止して人間判断を求める。発動したゲートと選んだ分岐は計測ログに記録する。
...
**証明 tier**: evidence の証明はローカルで再現可能な決定的テストを既定とする。環境・並行性・実プロセスに依存して決定的にできない挙動（process 終端、大規模並行、実インフラ依存）は、targeted テスト 1 本 + 「手動確認手順または deploy 後確認（該当機能は default-off）」の宣言で受理できる。flaky なテストを決定的にするための反復修正はしない（その時間は予算ゲートの対象になる）。
...
4. PR に round 1 のレビューコメント（妥当な指摘があった場合。main agent が妥当性判断後に投稿）と、レビューサマリー付きの最終コメント（APPROVED、または残指摘を明記した人間レビューへの引き継ぎ）がある
5. worker の evidence matrix が G3 の受理条件を満たし、main agent がレビューの妥当性を判断し、妥当な must-fix / should 指摘に対応済み、または予算ゲートに従って人間レビューへ引き継ぎ済み
...
- **検証 lease**: heavy validation（Gradle / Docker / Testcontainers 等を使うもの）は同一マシンで同時 1 本に直列化する。実行前に lock（例: `mkdir /tmp/issue-pr-autopilot/validation.lock` の成功を取得とみなす）を取得し、終了時に必ず解放する。lock 待ちの時間は検証台帳に記録する。JVM 系は worktree ごとに `GRADLE_USER_HOME` を分離し、他 run の Gradle daemon を停止させない。同一マシンで並走させる autopilot は 2 run までにする（超える場合はユーザーに申告する）。
...
<予算ゲートで打ち切った未解消の指摘を severity・閉じる条件付きで列挙する。なければこのセクションごと削除する>
...
- 妥当な must-fix / should: worker に修正させ、commit と push をさせる。**設計欠陥ラベル付きは worker でなく designer に設計修正を依頼し、issue の「## 設計」とノートを更新してから worker に反映させる**（同一 cycle 内の round として数える）。designer には設計 delta・falsifier 再確認の要否・**修正規模見積り（diff 行数・新規コンポーネント数）**を返させ、見積りが予算ゲート 3（diff 2 倍）または 4（新 layer / 新規サブシステム）に触れる場合は実装せず stage-out を既定とする: 該当機能を default-off / feature flag で隔離し、対策を follow-up issue の提案コメントとして投稿する。
...
must-fix / should がゼロになったら reviewer は**仮承認**（APPROVABLE_PENDING_FULL）を返す。**nit だけが残っている場合も、残存事項として注記した上で仮承認を返す。** 仮承認の時点では検証台帳は targeted tier で足りる（コードレベルの closure 確認に full を要求しない）。仮承認後に worker が最終 HEAD の full validation を 1 回だけ実行し、成功かつ SHA 不変なら確定に進む。full で失敗が出た場合のみ修正 + 再検証し、diff が変わったら reviewer に変更行だけ確認させる。

予算ゲートで打ち切った場合は、仮承認を待たずに「人間レビューへの引き継ぎ」ステータスで確定に進む（最終 full validation は同様に 1 回実行する）。

main agent は最終コメントの投稿前に次を照合する（G5）。

1. review_point_matrix に safety・migration・security の unchecked が残っていない。この unchecked は人間確認事項への転記（可視化）だけでは閉じない。閉じる方法は 3 つ: (1) 人間の回答・観測可能な証拠で reviewer が checked と再判定する（自走中は issue へ質問を 1 回投稿する。回答を無期限には待たない） (2) 該当範囲を分割して本 PR のスコープから除外する (3) 該当機能を default-off / feature flag で隔離し、隔離を reviewer が確認して checked と再判定する（有効化条件は人間確認事項へ書く）。どれも成立しない場合は APPROVED ではなく「人間レビューへの引き継ぎ」ステータスで確定し、unchecked を残指摘として明記する。
...
4. 最新 HEAD の required checks（GitHub CI）が完了して成功している（checks が無いリポジトリは「なし」と記録する）。pending 中は最終コメントを投稿せず `gh pr checks --watch` 1 回で待つ。failure は確定前に CI cycle として処理し、修正後の最新 HEAD で local full validation と required checks の両方を再確認する。外部承認や手動実行が必要な check を除外する場合は、その旨を人間確認事項として明示する。

成立したら次の形式の最終コメントを PR に投稿する。残指摘がある場合は見出しを「## レビュー結果: 人間レビューへの引き継ぎあり」に変え、残指摘（severity / 閉じる条件 / 引き継ぎ理由 = 発動した予算ゲート）の表を追加する。
