...
- 変更が複数レイヤーに跨る場合、ユーザーに質問できる状況なら、質問ツールで「(a) 設計サブエージェントを走らせてから実装（推奨）/ (b) 設計なしで進める / (c) 中断して設計セッションで行う」を確認する。自走中で質問できない場合は (a) を選ぶ。
...
コア原則: 役割ごとにモデルティアと effort を分ける。多ターンでトークンを消費する実装 worker にはバランスティアの高 effort を、1 パスで品質が決まる設計サブエージェントと round 1 レビューには最上位ティアの高 effort を割り当て、round 2 以降の差分検証は同じ reviewer のまま低 effort に落とす。diff・コンテキストノート・既存コードを長く読む役割には最軽量ティアを使わない（長文脈の読み落としが増える）。サブエージェントを spawn・継続するたびに、割り当てたモデル名と effort をユーザーへの進捗報告に明示する。サブエージェントは数を増やすより、1 回あたりの入力を絞る方が効く。
...
- 推奨割当: 実装 worker = `gpt-5.6-terra`（`model_reasoning_effort = "high"`）、設計サブエージェント / reviewer round 1 = `gpt-5.6-sol`（`"xhigh"`）、round 2 以降は同一スレッドで `"medium"`。`gpt-5.6-luna` は長文脈の読解が弱いため、context をほとんど積まない機械作業以外に割り当てない。
