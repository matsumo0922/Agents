...
情報源の優先順位は「このセッション内の設計議論・設計レビュー > issue 本文とコメント > main agent の推定」とする。同一セッションで設計議論が行われている場合、その結論は要約ではなく決定事項としてすべて転記する。issue 本文またはコメントに「## 設計」セクションがある場合は、その内容を取り込む。転記後、サブエージェントを spawn する前に「会話・issue で合意した設計判断のうちノートに載っていないものはないか」を確認する。
...
- ノートの「設計コンテキスト」に記載された設計・決定事項に従って実装してください。設計の簡略化や別アプローチの方が良いと考えた場合は、実装を進める前に main agent に理由付きで報告し、判断を仰いでください。黙って簡略化・別解で実装することを禁止します。
...
- 機能名・クラス名・コマンド名を追加・変更した場合は、影響するドキュメント（README / docs/ / KDoc）を同じ PR 内で更新してください。変更した名前で docs/ と README を grep し、誤りになった記述が残っていないことを確認してください。ドキュメントは現在の仕様を現在形で書き、変更の経緯や将来の予定を書かないでください。
- 関数のシグネチャや呼び出し規約を変えた場合は、呼び出し元を grep で全件洗い出して追随してください（テストコード含む）。
- 新しい安全機構・新機能を追加した場合は、手組みの入力だけで通る unit テストに頼らず、本番の配線経由で実際に発動することを確認するテストを含めてください。
- レビュー指摘（must-fix / should）の修正では、指摘された箇所だけの対症療法で終わらせず、根本原因を特定し、同じ根の問題が他の層・他の箇所に残っていないか grep で確認して一括で修正してください。
- push の直前に `git fetch origin <base-branch>` と `git rebase origin/<base-branch>` を実行してください。conflict は自分で解消し、rebase で HEAD が変わった場合は検証を 1 回やり直して検証台帳を更新してください。
- 報告前にセルフレビューを行ってください: 受け入れ条件の充足、規約違反の有無、スコープ逸脱の有無、diff の最終確認、コミット粒度、ドキュメント影響（grep 確認済みか）。lint や規約レベルの問題はレビューに回さず、この段階で直してください。
...
コア原則: 役割ごとにモデルティアと effort を分ける。多ターンでトークンを消費する実装 worker にはバランスティアの高 effort を、1 パスで品質が決まる round 1 レビューには最上位ティアの高 effort を割り当て、round 2 以降の差分検証は同じ reviewer のまま低 effort に落とす。diff・コンテキストノート・既存コードを長く読む役割には最軽量ティアを使わない（長文脈の読み落としが増える）。サブエージェントは数を増やすより、1 回あたりの入力を絞る方が効く。

### Claude Code

- サブエージェント呼び出し時の `model` / `effort` パラメータで役割別に調整する。推奨割当: 実装 worker = Sonnet 5（`effort: high`）、reviewer round 1 = Opus 4.8（`effort: high`）、round 2 以降は同じ reviewer を `low`〜`medium` で継続する。
...
- 推奨割当: 実装 worker = `gpt-5.6-terra`（`model_reasoning_effort = "high"`）、reviewer round 1 = `gpt-5.6-sol`（`"xhigh"`）、round 2 以降は同一スレッドで `"medium"`。`gpt-5.6-luna` は長文脈の読解が弱いため、context をほとんど積まない機械作業以外に割り当てない。
- `model_reasoning_effort` の `ultra` は使わない。ultra はモデル内部でサブエージェントを並列 spawn するため、本 skill のオーケストレーションと二重になりトークン消費だけが増える。
