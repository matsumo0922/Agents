...
description: "GitHub issue の説明やリンクを起点に、兄弟ディレクトリの worktree で実装し、適切な粒度でコミット、PR 作成、実装 worker と reviewer サブエージェントによるレビュー反復、evidence の受理ゲート、レビュー妥当性判断、追加修正、push、最終コメント（APPROVED または残指摘の人間レビュー引き継ぎ）まで自走する。生成物は issue の受け入れ条件への紐付け（意図アンカー）を必須とし、内部レビューは収束性で継続を判定して、収束が止まったら人間レビューへ引き継ぐ。品質保証の最終責任は人間の PR レビューが担う。Use when the user wants an agent to turn an issue or task description into a reviewed pull request with autonomous subagents, worktrees, commits, and a review loop."
---
...
本スキルは dig / design / issue-pr-autopilot / claude-rescue からなるパイプライン bundle の一部として配布される（`make link` で一括リンク。単体配布はサポートしない）。設計の形式は design スキルの `references/design-contract.md`（設計契約）が正本で、本スキルはその構造検査に必要な最小限だけを「gate schema」として持つ。claude-rescue は Claude サブエージェントを持たない環境で reviewer / falsifier を呼び出すブリッジとして使う（環境別ヒント参照）。
...
- **意図アンカー**: すべての生成物（設計決定・レビュー指摘・修正 diff）は「issue の受け入れ条件・設計 invariant のどれを守るためか」の紐付けを持つ。invariant には「今回の変更が既存の正しい挙動を壊さない」という暗黙の非退行 invariant を含む（今回導入・悪化させた regression はこれにアンカーできる）。紐付けられない指摘は must-fix にできず、紐付けられない変更は diff に含めない（follow-up issue 提案に回す）。
- 内部レビューの継続・停止は round 数でなく収束性で判定する（「収束判定と HANDOFF」参照）。権限や外部依存で進めない場合、仕様判断が必要な場合は未解決事項として報告する。
- skill 改訂は開始済みの run には反映されない。改訂を適用する場合は run を再起動する（main は run 開始時に読んだ skill の版をノートに記録する）。

### 収束判定と HANDOFF

数値の停止閾値（round 上限・PARTIAL 回数・diff 倍率・時間 SLO）は置かない。ループの継続・停止は、main agent が round ごとに次の収束性で判定する。

- **収束している**: 未解消 must-fix 数が前 round から減少しており、新規指摘（あれば）が意図アンカーを満たす。→ 続行する。収束している限り round 数で打ち切らない。
- **収束停滞**: 未解消 must-fix 数が減っていない（横ばい・増加）、同一指摘の PARTIAL 判定が繰り返されている、または対応が閉じる条件の inventory 外へ拡張し続けている。→ 修正を積み増さず HANDOFF する。
- **人間専権**: 残った must-fix / unchecked が人間の回答・権限なしには閉じられない。→ HANDOFF する（自走中は issue への質問投稿を 1 回試みてよい。回答を無期限には待たない）。

指摘への対応に新しい layer の追加・新規サブシステムの設計が必要になった場合は、修正の積み増しでなく stage-out を既定とする（レビュー反復の節）。

**規模の申告義務**: designer は設計に、worker は完了報告に、規模（変更ファイル数・diff 行数・failure domain 数・新規 state machine 数・fixture / selftest のコスト）を申告する。申告値は自動停止条件に使わず、main の裁定・人間への報告・計測ログの材料とする。

**分岐**: 対話中はユーザーに「続行 / HANDOFF（残指摘を人間レビューへ引き継いで終了） / stage 分割」を質問する。自走中の既定は **HANDOFF**。ただし security・データ破壊級の must-fix を未修正のまま HANDOFF はできない: 該当機能を default-off / feature flag / stage-out で PR から隔離してから終了し、隔離もできない場合のみ停止して人間判断を求める。HANDOFF の判定根拠（未解消 must-fix 数の推移、または人間専権事項）は計測ログに記録する。
...
- main agent が自分で行うのは、goal 設定、事前調査（gh によるメタ情報取得と reference 解決確認）、コンテキストノート作成、worktree 管理、`git log --oneline` と `git diff --stat` によるスポットチェック、受理ゲート G2〜G4 の構造検査と G5（APPROVED 条件）の照合、falsifier の発動判定、PR 操作（description 更新を含む）、レビュー妥当性判断（4 分類裁定）、対応表と計測ログの管理、最終報告だけ。
...
**stage 予算**: 変更ファイル 〜25 / diff 〜1,500 行 / binding decision 〜10（正本は設計契約のスコープ判定）。超過したまま進む判断は designer 単独では行えず、main の裁定事項とする。
...
1. 行 = 受け入れ条件・設計の不変条件・（レビュー修正時は）裁定済み must-fix の閉じる条件に 1:1 対応している。どれにも紐付かない行を作らない。
...
5. worker の evidence matrix が G3 の受理条件を満たし、main agent がレビューの妥当性を判断し、妥当な must-fix / should 指摘に対応済み、または収束判定に従って HANDOFF として記録済み
6. 検証台帳の最終エントリが「最終 HEAD の full validation 成功」で SHA が HEAD と一致している。HANDOFF 終了の場合は、最終 HEAD の full validation を 1 回試行し、その結果（失敗を含む）が残指摘に記録されていれば満たす
7. PR description の検証セクションが最終 HEAD と同期し、最新 HEAD の required checks が成功している（checks が無い場合は「なし」と記録）。HANDOFF 終了の場合は、checks の最終状態が HANDOFF セクションに記録されていれば満たす
...
- **bundle 完全性の確認（fail-fast）**: この SKILL.md があるディレクトリからの相対で `../design/references/design-contract.md`・`../design/references/falsifier-rubric.md`・`references/review-rubric.md` を絶対パスへ解決し、存在を確認する。**claude-rescue ブリッジを使う環境（Claude サブエージェントを持たず reviewer / falsifier を Opus 4.8 で呼ぶ環境。環境別ヒント参照）では、加えて `../claude-rescue/scripts/claude-bridge.sh` も確認する**（Claude Code は Agent tool で直接 spawn するため不要）。解決できない場合は bundle 破損として設計ゲート以降へ進まず、`make link` の再実行をユーザーに要求して停止する。解決した絶対パスと、run 開始時に読んだ skill の版（bundle リポジトリの commit SHA）をコンテキストノートに記録し、サブエージェント指示に埋め込む。
...
- 関連コードを読み、設計契約の「## 設計」形式・記入基準（事実と仮定の正本分離、スコープ判定の客観 signal、条件付きセクションの発動判定と非該当宣言）に従って設計する。
- 決定は「〜とする」の形で書き、各決定に「どの受け入れ条件・要件を守るための決定か」の紐付け（意図アンカー）を付ける。high リスクかつ未検証の仮定は、まず自分でコード・保存状態・運用経路から検証を試み、残ったものを理由付きで列挙する（勝手に仮決めしない）。
...
2. 変更予定ファイル map の要約（**変更予定ファイル数を明示する**）と、設計が触れる範囲（layer 数 / safety / migration / security / consumer 数 / DB hot path の該当有無）
3. high リスク・未検証のまま残った前提の一覧。なければ「なし」
4. 発見事項: 調査中に見つけた、受け入れ条件に紐付かない欠陥・改善点の列挙（各 1 行）。設計には含めない。なければ「なし」
</deliverables>
</designer_instruction>
```
...
- main agent が deliverable 2 を gate schema の客観条件リストに照らして判定し、該当すれば clean context の falsifier を 1 体 spawn する（必須。designer 自身に「該当なし」と判断させない。迷ったら発動）。falsifier には設計セクション全文・`falsifier-rubric.md` の絶対パス・対象リポジトリ（読み取りのみ）だけを渡し、rubric の職掌 5 点を falsification_result 形式で返させる。該当しない場合は designer に rubric に従った自己反証を別ターンで依頼する。自己反証で blocking 反例が 1 件でも出た場合は独立反証に昇格し、修正後の解消確認も独立 falsifier が行う。
...
反証パスの完了後、main agent は designer の deliverable 2 の変更予定ファイル数を gate schema の stage 予算に照らす。

- **予算内**: そのまま worker の spawn へ進む。
- **超過**: designer へ再分割を差し戻す（既定）。designer が「理由を書いて超過のまま進む」ことは認めない。再分割が成立しない（各 stage が単独 merge・deploy 可能かつ backward-compatible にできない）と designer が申告した場合に限り、main が超過続行を裁定してよく、その判断と根拠を計測ログに記録する。
- 予算は品質の下限ではなく、下流の実装・レビュー・検証の面積を有界にするための上限として扱う。予算内に収めるために受け入れ条件を落とさない（落とす必要がある場合は stage 分割か、受け入れ条件そのものの人間判断が要る）。

worker の完了報告時点で規模の申告が予算を大きく超えた場合も、main は同じ基準で stage-out を検討する（判断は「収束判定と HANDOFF」に従う）。
...
- **検証 lease**: heavy validation は同一マシンで同時 1 本に直列化する。lease の対象は列挙で定める: Gradle 系タスク、`docker build`、Testcontainers を使う test suite、使い捨て DB の起動、exact-image canary、selftest 類。lease 操作は各 agent が shell で即興実装せず、同梱の helper `scripts/validation-lease.sh`（本 SKILL.md からの相対パス。main が絶対パスへ解決してノートに記録する。python3 依存）で検証コマンドを包んで実行する。helper は OS の advisory lock（flock）を使うため、lock の取得・解放・クラッシュ時解放を kernel が保証し、stale 判定・owner 管理・強制解放は不要。lock fd は検証プロセスにも継承されるため、wrapper が先に死んでも（SIGKILL を含む）検証プロセスが生きている限り lock は保持され、新しい lease 保持者との並走は起きない。SIGINT / SIGTERM / SIGHUP は検証プロセスの process group へ転送され、子の終了を待ってから解放される。helper が出力する lock 待ち時間は external wait として台帳に記録する。lock 待ちが長い場合はユーザーへ通知する（時間を理由に他 run の lease を奪わない）。helper が使えない環境では mkdir ベースの lock で代替してよいが、orphan lock の強制削除は自動化せず人間に確認する。JVM 系は worktree ごとに `GRADLE_USER_HOME` を分離し、他 run の Gradle daemon を停止させない。同一マシンで並走させる autopilot は 2 run までにする（超える場合はユーザーに申告する）。
- **復旧操作の規約**: マシン共有状態に触る復旧操作（共有 GRADLE_USER_HOME への `gradlew --stop`、`docker system prune`、プロセスの一括 kill 等）を worker が独断で行わない。自分の隔離資源（自分の `GRADLE_USER_HOME`・自分の worktree）内で完結しない復旧は main へエスカレーションする。

## 実装 worker への指示

main agent は実装 worker サブエージェントを明示的に spawn し、次の形式で依頼する。利用環境のサブエージェント機能に合わせて文言だけ調整してよい。**レビュー指摘の修正は初回実装の worker に継続させず、finding cluster ごとに fresh の修正 worker を spawn する**（モデル割当は環境別ヒント）。修正 worker には該当指摘・対象ファイル・ノートの該当節だけの狭い入力を渡す。長時間稼働でコンテキスト圧縮（compaction）を跨いだ worker は目的関数が「issue のゴール」から「直近の指摘リスト」へずれるため、修正を継続させない。
...
<rules>
禁止: main checkout・他 worktree の変更 / 頼まれていない修正・テスト・リファクタ・hardening の追加（無関係なリファクタ・別 issue の先取りを含む）/ 秘密情報の追加 / 自分のサブエージェントの spawn / issue 本文の再取得（gh issue view の再実行）/ GitHub PR 操作（PR の作成・description 編集は main agent の責務）/ 検証が失敗した状態での完了報告（解決できない場合は失敗理由を報告する）。

- まずコンテキストノートを読み、「設計コンテキスト」の設計・決定事項に従って実装する。簡略化や別アプローチの方が良いと考えた場合は、実装を進める前に理由付きで main agent に判断を仰ぐ（黙って別解にしない）。
- diff に含めるのは、受け入れ条件・設計決定・割り当てられた指摘の閉じる条件に紐付く変更だけとする。作業中に見つけたスコープ外の欠陥・改善点は修正せず、完了報告で報告のみ行う（follow-up issue 化は main が裁定する）。
...
2. commit 一覧（SHA と message）、変更ファイル一覧の 1 行要約、規模の申告（変更ファイル数・diff 行数・failure domain / 新規 state machine 数・fixture / selftest のコスト）
...
- 網羅性を簡潔さより優先する。最初の該当指摘で止まらず、該当する指摘をすべて列挙し切ってから返す（取捨選択は下流の main が行う）。
- **各指摘に意図アンカーを付ける**: 「issue の受け入れ条件・設計 invariant のどれを守るための指摘か」の紐付け。今回の diff が導入・悪化させた correctness / security / safety / 互換性の regression は、暗黙の非退行 invariant にアンカーできる（既存・未変更の欠陥は不可。定義は rubric）。紐付けられない指摘は内容の正しさに関わらず must-fix にできない: follow-up 提案として分類して報告する（報告自体は禁止しない。分類を強制する）。
- must-fix / should / nit の 3 段 triage をこの round で確定させる。**nit の既定処置は「対応不要（記録のみ）」**とする（nit 対応は diff を増やしてレビュー面積を再拡大させるため）。
- must-fix の severity 基準・同根 sweep・有限の閉じる条件（ID 付き inventory + 証明する観測）は rubric に従う。設計自体の欠陥は「設計欠陥」ラベル付き must-fix にする。
...
- findings: <各指摘の要約（severity / file:line / 要旨 / 意図アンカー（対応する受け入れ条件・invariant。無ければ follow-up 提案に分類）/ must-fix は閉じる条件と同根 call site 一覧も含める）>
...
- 再レビューの範囲は `git diff <last_reviewed_sha>..HEAD` の変更行と、前回指摘の対応確認。対応確認は各 must-fix の inventory ID 単位で **CLOSED / PARTIAL（未充足 ID を列挙）/ NEW** で判定し、**同一指摘の要求を後続 round で拡張しない**（inventory に無い経路を後から見つけた場合は、reopen ではなく NEW の新規指摘として扱う）。新規指摘は意図アンカーを満たす場合のみ must-fix にでき、アンカー無しは follow-up 提案に分類する。新規 must-fix が round を跨いで出続ける場合、main はそれを収束停滞のシグナルとして収束判定に入力する。修正コミットが新規に持ち込んだ問題（rubric の修正コミット固有欠陥クラス）は round に関わらず指摘対象。無関係な未変更コードの再レビューは行わない。
- **round ごとの収束判定**: main は再レビューの返却ごとに未解消 must-fix 数の推移を計測ログへ記録し、収束判定（「収束判定と HANDOFF」）を行ってから次の round へ進む。
...
- ルール: 前回指摘は inventory ID 単位で CLOSED / PARTIAL / NEW を判定し、同一指摘の要求を拡張しない。新規指摘は意図アンカー必須（無ければ follow-up 提案に分類）。無関係な未変更コードの再レビューは不可。build / test は実行しない。結果は GitHub に投稿せず review_result 形式で返す。
</rereview_request>
```
...
- **HANDOFF（人間レビューへの引き継ぎ）**: 収束停滞・人間専権による停止、または G5 を満たせない場合の内部処理の終了。**承認ではなく、G5 通過とも扱わない。** 残指摘・unchecked・cycle 表を PR に記録し、判断を人間に引き渡す。
...
成立したら次の形式の最終コメントを PR に投稿する。HANDOFF の場合は見出しを「## レビュー結果: HANDOFF（人間レビューへの引き継ぎ。承認ではない）」に変え、残指摘（severity / 閉じる条件 / 理由 = 収束停滞の推移・人間専権事項・または満たせなかった G5 条件）の表を追加する。
...
- **cycle 判定の validator**: 内部レビューの再往復（worker 修正 → 同一 reviewer の再レビュー）は、上記トリガーに該当しない限り常に**同一 cycle の次 round** として数える。routine な再レビューを新 cycle として扱うと、収束判定（未解消 must-fix 数の推移）と incomplete closure の集計が無効になるため、新 cycle の開始時は該当トリガーを計測ログに必ず記録する（トリガーを書けない cycle 開始は誤分類）。
- cycle 開始時に reviewer へ「新 cycle・トリガー・レビュー範囲」を明示して依頼する。外部指摘は鵜呑みにせず reviewer に再監査させる。
- **計測ログ**: main agent は round ごとに即時記録する（review 返却時と worker 修正完了時。最終確定まで書き溜めない）。コンテキストノートの専用セクションに cycle ごとに 1 行で記録する: cycle 番号 / トリガー / round 数 / must-fix・should 件数と round ごとの未解消 must-fix 数の推移 / finding origin（internal-r1 / internal-r2+ / external / CI）/ fix-induced regression 件数 / incomplete closure（同一指摘の再往復）件数 / 意図アンカー違反件数（アンカー無し指摘・アンカー外 diff の検出数）/ 役割別の使用モデルと G3 差し戻し回数（修正 worker の pilot 比較用）/ HANDOFF 判定の根拠（該当時）/ 時間。時間は 4 区分: wall-clock elapsed（cycle 開始から終了までの実時間）/ cumulative agent compute time（並列実行を含む全サブエージェント実行時間の合計）/ critical-path agent elapsed（直列依存の経路上にあるエージェント実行時間。算出困難なら unknown とし、推測値を確定値として記録しない）/ external wait（ユーザー・外部レビュー・CI の待ち時間）。cycle 表は最終コメントと最終報告に転記するほか、**run を中断・引き継ぎする場合もその時点の表を PR コメントへ投稿する**（/tmp は揮発のため PR コメントが永続記録になる）。
...
コア原則: **細部への目配りは設計では資産、レビューでは負債になる。** 役割を「判断（1〜2 パスの較正の質で決まる: reviewer / falsifier）」「牽引（長文脈の grind: designer / 実装 worker）」「限定実行（狭い入力・明確な完了条件: 修正 worker / 機械的タスク）」に分けて割り当てる。現行世代のモデルは手順の細目列挙より「ゴール + 制約 + 完了条件」で最も性能が出るため、サブエージェント指示はテンプレートの必要部分だけを埋め、ノートにある情報を重複転記しない。reviewer の round 1 網羅方針（確度の低い指摘も報告させ、フィルタは main の裁定で行う）は recall を上げるための設計であり、reviewer 側で保守的に間引かせない。サブエージェントは数を増やすより 1 回あたりの入力を絞る方が効く。spawn・継続のたびに、割り当てたモデル名（effort を指定できる環境では effort も）を進捗報告に明示する。

**役割別割当（既定）**: 役割ごとにモデル特性で割り当てる。同じ役割でも環境ごとに使えるモデルが違うため、Codex / Claude Code 列に分けて具体モデルを示す。

| 役割 | Codex | Claude Code | 補足 |
|---|---|---|---|
| main（裁定・進行） | セッションのモデル | セッションのモデル | 裁定は 4 分類で定型化しモデル依存を下げる |
| designer | gpt-5.6-sol / high | Opus 4.8 / high | 細部の目が届く設計。膨張は契約側（スコープ判定・stage 予算・stage-out・規模申告）で抑制 |
| falsifier | Opus 4.8 / high（ブリッジ経由） | Opus 4.8 / high | 実質「設計時の reviewer」。核心は反例の検出でなく blocking の較正 |
| reviewer | Opus 4.8 / high（ブリッジ経由） | Opus 4.8 / high | 必要なのは検出力でなく triage 力。designer / worker と別ベンダーで盲点が非相関 |
| 実装 worker（primary） | gpt-5.6-sol / high | Sonnet 5 / high | 長文脈 grind。品質がレビュー round 数に直結 |
| 修正 worker（cluster ごと fresh） | gpt-5.6-luna / max（pilot、fallback sol/high） | Sonnet 5 / high | 狭い入力 + 明確な閉じる条件。Codex の luna pilot は計測ログ（G3 差し戻し・意図アンカー違反・round 増加）で sol/high と比較し実運用で昇降格を決める |
| 機械的・自明タスク | gpt-5.6-luna / high | Haiku 4.5 / high | — |

- **Claude Code では全役割を Agent tool で直接 spawn し、claude-rescue ブリッジは使わない**（ブリッジは Claude サブエージェントを持たない環境＝ Codex 等が Opus 4.8 を呼ぶための橋）。Codex 列の「ブリッジ経由」がその発生箇所。
- effort は Codex が per-spawn 指定、Claude Code は per-spawn 不可のため `.claude/agents/` で high 固定。

**Claude Fable 5 の発動条件**: 次のいずれかに該当する場合のみ使う（run あたり 0〜2 回の単発呼び出しに限る）。該当しなければ Opus 4.8。

1. safety / security / 不可逆 migration が主対象の設計への falsifier
2. 収束停滞から HANDOFF 判定に入る前の、最後の 1 回の deep review（「この PR の本質的な問題は何か」を 1 パスで切る）
3. reviewer と designer / worker の判断が割れて main が裁定できない場合の仲裁 1 回
4. ユーザーの明示指示

**effort 原則**: 全役割 high を既定とし、深さが足りない場合は effort ではなくモデルを上げる（Opus high → Fable high）。例外は Luna のみ max を許可する（effort スケールが急峻なため）。Sol の xhigh / max は使わない。

### Claude Code

- **全役割を Agent tool で直接 spawn し、割当表の Claude Code 列（designer / falsifier / reviewer = Opus 4.8、実装・修正 worker = Sonnet 5、機械的 = Haiku 4.5、Fable 5 は発動条件のみ）を `model` パラメータで指定する。** GPT 系は spawn できないため使わず、claude-rescue ブリッジも使わない（ブリッジは Claude サブエージェントを持たない環境専用で、Claude Code では Agent tool が上位互換）。effort は per-spawn 指定できないため、役割別に固定する場合は `.claude/agents/` の role 別エージェント定義で high にする。
- falsifier / 修正 worker は clean context のため新規 spawn する。round 2 以降の reviewer は SendMessage で継続し、入力を前回 findings と `git diff <last_reviewed_sha>..HEAD` に絞る。
...
- GPT 系の役割は `~/.codex/agents/` の TOML（`model` / `model_reasoning_effort`）で割当表どおりに定義する。並列上限は `agents.max_threads`（既定 6）。
- **reviewer / falsifier（Opus 4.8）の呼び出しは claude-rescue skill の `claude-bridge.sh` で行う**（bundle 完全性チェックで解決した絶対パスを使う）。運用規則:
  - ブリッジ呼び出しは常に `require_escalated` で起動する（session 保存と `--resume` 継続に `~/.claude/projects` への書き込みが必要）。
  - round 2 以降は初回呼び出しの stderr の session_id を `--resume` に渡して同一レビュアーを継続する。escalation が承認されない・resume が失敗した場合は、前回 findings + 対応表 + `git diff <last_reviewed_sha>..HEAD` を instruction に含めた stateless 再依頼へ切り替える。
  - ブリッジ不通（exit 3）時の fallback は `gpt-5.5 / high`（sol ではない）。
  - Fable 5 エスカレーション（発動条件参照）は `--model claude-fable-5` の上書きで行う。
  - reviewer / falsifier の instruction には出力契約（review_result / falsification_result）を書き、`--expect review_result` 等を付ける。
- 本 bundle の運用規約として、Codex では main もその run で必要になった reference だけを読んでよい。設計ゲートを通らない run では design-contract.md を読まない。
- clean context が必要な役割（修正 worker 等）は fork なしで spawn する（確認済みの指定は `spawn_agent` の `fork_turns = "none"`。fork 制御のパラメータ名・値域は実行環境で確認してから使う）。
- **サブエージェント操作の tool 名は世代で異なる。固有名を仮定せず、実行環境で公開されている collaboration tool を最初に確認して分岐する**: hosted surface では `spawn_agent` / `send_message` / `followup_task` / `wait_agent` / `interrupt_agent` / `list_agents` を使い、継続は `followup_task`、状態確認は `wait_agent` / `list_agents` で行う。local legacy surface では `send_input` / `resume_agent` / `close_agent` が実在する場合のみ使い、読み終えた終了済み subagent は `close_agent` で閉じる（spawn スロットのリーク対策）。
- **進捗確認は節目ベースの worktree 観察で行う**: 固定の短間隔ポーリング（60 秒間隔の wait 繰り返し等）は main の巨大コンテキストの再処理を繰り返すため行わず、まとまった 1 回の wait と、commit 等の節目での読み取り専用観察（`git -C <worktree> log --oneline` / `status -sb` / `diff --stat`）を組み合わせる。wait のタイムアウトは「まだ Running」という意味でしかなく、ハングの根拠にしない。実装が長時間になるのは正常。
- Running 中の worker にメッセージで進捗を尋ねない（user 入力として注入されターンを乱す）。interrupt は実行中ターンを中断させ、作業を失わせる。停滞判定は経過時間ではなく worktree の変化で行う: 間隔を空けた複数回の観察で log・status・diff に変化がない場合に限り、interrupt なしのメッセージを 1 回だけ送り、それでも変化がなければ再 spawn する。main agent が実装を引き継がない。
- 組み込みの `/review` は read-only の専用レビュアーで、指摘の洗い出しに使える（最終的な triage・投稿は本 skill のプロトコルに従う）。
