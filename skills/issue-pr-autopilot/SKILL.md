...
- **検証 lease**: heavy validation（Gradle / Docker / Testcontainers 等を使うもの）は同一マシンで同時 1 本に直列化する。lock は `mkdir /tmp/issue-pr-autopilot/validation.lock` の成功を取得とみなし、直後に owner 情報（run slug / PID / 取得時刻）を `validation.lock/owner` に書く。worker は検証コマンドと解放を 1 つの呼び出しに束ね、成功・失敗・中断（trap）のすべてで解放する。**stale 判定は owner PID の生存のみで行う**: owner PID が生存している限り、経過時間を理由に解放しない（長時間の正常な検証を破棄して heavy validation を並走させないため。lock 待ちが長い場合はユーザーへの通知のみ行う）。owner PID が生存していない場合は、owner ファイルを再読して取得時と同一 owner であることを確認してから削除する（compare-and-delete。確認と削除の間に新 owner へ入れ替わる競合を防ぐ）。強制解放の事実は検証台帳に記録する。lock 待ちの時間は external wait として台帳に記録し、SLO 通知の 120 分には算入しない。JVM 系は worktree ごとに `GRADLE_USER_HOME` を分離し、他 run の Gradle daemon を停止させない。同一マシンで並走させる autopilot は 2 run までにする（超える場合はユーザーに申告する）。
