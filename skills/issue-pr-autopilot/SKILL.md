...
- モデルはサブエージェント呼び出し時の `model` パラメータで役割別に指定する。推奨割当: 実装 worker = Sonnet 5、designer / falsifier / reviewer = Fable 5。Haiku 4.5 は機械的な補助作業に限り、長文脈読解を伴う役割に使わない。falsifier は clean context のため新規 spawn する。
- effort は per-spawn で指定できない（session 既定を継承する）。役割別に effort を固定したい場合は `.claude/agents/` の role 別エージェント定義（frontmatter）で指定する。spawn 済みスレッドの model / effort は SendMessage 継続でも変更できない。round 2 以降の消費削減は effort 変更ではなく、同一 reviewer の継続と入力の絞り込み（前回 findings と `git diff <last_reviewed_sha>..HEAD` のみ）で行う。
- security・情報露出が主対象の変更では reviewer に Opus 4.8 を割り当てる。Fable 5 の reviewer が security 分析で refusal・分析劣化を起こした場合も Opus 4.8 で再依頼し、unchecked のまま G5 停止に入る不要な経路を作らない。
