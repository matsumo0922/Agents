...
- **検証 lease**: heavy validation（Gradle / Docker / Testcontainers 等を使うもの）は同一マシンで同時 1 本に直列化する。lock は `mkdir /tmp/issue-pr-autopilot/validation.lock` の成功を取得とみなし、直後に owner 情報（run slug / PID / 取得時刻）を `validation.lock/owner` に書く。解放は検証コマンドの成否・中断に関わらず必ず行う（コマンドと解放を 1 つの呼び出しに束ねる）。**stale 判定**: lock 待ちの際に owner を確認し、owner PID が生存していない、または取得から 90 分を超えている場合は stale として強制解放し、その事実を検証台帳に記録する（クラッシュした run の lock が全 run を永久待ちにさせない）。lock 待ちの時間は external wait として台帳に記録し、SLO 通知の 120 分には算入しない。JVM 系は worktree ごとに `GRADLE_USER_HOME` を分離し、他 run の Gradle daemon を停止させない。同一マシンで並走させる autopilot は 2 run までにする（超える場合はユーザーに申告する）。
