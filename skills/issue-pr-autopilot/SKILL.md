...
| 役割 | モデル / effort | 補足 |
|---|---|---|
| main（裁定・進行） | セッションのモデル | 裁定は 4 分類で定型化し、モデル依存を下げる |
| designer | gpt-5.6-sol / high | 細部まで目の届く設計はエッジケースの取りこぼしを減らす。膨張は契約側（スコープ判定・stage 予算・stage-out・規模申告）で抑制する |
| falsifier | Claude Opus 4.8 / high | 実質「設計時の reviewer」。核心能力は反例の検出でなく blocking の較正 |
| reviewer | Claude Opus 4.8 / high | 必要なのは検出力でなく triage 力。designer / worker と別ベンダーのため盲点が非相関 |
| 実装 worker（primary） | gpt-5.6-sol / high | 長文脈 grind。worker の品質はレビュー round 数に直結する |
| 修正 worker（cluster ごと fresh） | gpt-5.6-luna / max（pilot） | 狭い入力 + 明確な閉じる条件。fallback = sol / high。計測ログ（G3 差し戻し・意図アンカー違反・round 増加）で sol / high と比較し、昇格・降格は実運用計測で決める |
| 機械的・自明タスク | gpt-5.6-luna / high | |
...
- モデルはサブエージェント呼び出し時の `model` パラメータで指定する。GPT 系を使えない構成では、designer / falsifier / reviewer = Opus 4.8、実装 worker = Sonnet 5、機械的タスク = Haiku 4.5 で代替する（Fable 5 は発動条件のみ）。effort を役割別に固定する場合は `.claude/agents/` の role 別エージェント定義で high に固定する。
