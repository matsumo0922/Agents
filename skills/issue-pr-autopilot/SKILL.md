...
- must-fix を 1 件見つけたら、それを問題クラスとして一般化し、同じクラスの他のインスタンス（他の層・経路・出力先）を grep と call graph 追跡で列挙して、全該当箇所を 1 つの指摘グループとして報告する。1 経路だけ報告して修正後に隣の経路を指摘する進め方を禁止する。
- 各 must-fix には「閉じる条件」を必ず書く: 修正後に成立しているべき不変条件と、テストが証明すべき境界。症状の記述だけで返さない（閉じる条件が曖昧な指摘は worker の部分修正と追加ラウンドを生む）。
...
- severity の基準: must-fix = 受け入れ条件違反、または発生条件を特定できる証明可能な欠陥（データ破壊・レース・セキュリティ・互換性・情報露出）。should = 受け入れ条件は満たすが品質・保守性に実害がある。nit = 好みや微細な改善。
...
- findings: <各指摘の要約（severity / file:line / 要旨 / must-fix は閉じる条件も含める）>
...
- 例外: must-fix の修正確認では、修正が対症療法で終わっていないか（同根の問題が他の層に残っていないか）の確認に必要な範囲で、関連する未変更コードを参照してよい。ただし新規指摘は同根の問題に限る。同根の新規指摘は round 1 の網羅漏れとみなし、なぜ round 1 で検出できなかったかを 1 行添える。
...
- ルール: 未変更コードの再レビューと新規指摘は禁止（修正コミットが持ち込んだ問題と、must-fix 修正の根本対処確認で見つかる同根の問題を除く）。同根の新規指摘には round 1 で検出できなかった理由を 1 行添える。build / test は実行しない。結果は GitHub に投稿せず review_result 形式で返す。
</rereview_request>
```
...
コア原則: 役割ごとにモデルティアと effort を分ける。設計とレビューは最上位ティアの高 effort を使い、多ターンの実装 worker は速度と消費を優先する（バランスティアの高 effort、またはトークン効率に優れる最上位ティアの中 effort）。reviewer は round 1 で指摘を出し尽くし（同根クラスの全経路を 1 グループで報告し、各指摘に閉じる条件を付ける）、round 2 以降は同じ reviewer を継続して、前回指摘の対応確認と新規 diff だけに入力を絞る。Codex では spawn 後の effort を途中変更できると仮定しない。実行中のサブエージェントの進捗は、メッセージ送信ではなく worker worktree の読み取り専用観察（git log / status / diff）で確認し、実装が長時間になることをハングと判断しない。diff・コンテキストノート・既存コードを長く読む役割には最軽量ティアを使わない。サブエージェントを spawn・継続するたびに、割り当てたモデル名と effort をユーザーへの進捗報告に明示する。サブエージェントは数を増やすより、1 回あたりの入力を絞る方が効く。
...
- 推奨割当: main agent = `gpt-5.6-sol`（`model_reasoning_effort = "medium"`、`plan_mode_reasoning_effort = "high"`）、実装 worker = `gpt-5.6-sol`（`"medium"`。トークン効率が高く、`gpt-5.6-terra` の `"high"` と同等以下のステップ数・出力量でより高い精度が出るため、実装の速度優先と両立する）、設計サブエージェント / reviewer = `gpt-5.6-sol`（`"xhigh"`）。
...
- `wait` の `timed_out: true` は「まだ Running」という意味でしかなく、ハングの根拠にしない。`timeout_ms` は打ち切りではなくブロックする長さのため、長め（10 分以上）に明示し、タイムアウトしたら観察を挟んで再度 `wait` する。実装が長時間になるのは正常。
...
- Running 中の worker に `send_input` で進捗を尋ねない。メッセージは worker に user 入力として注入され、返信は worker のターン終了時（`Completed(final_message)`）にしか届かない。`interrupt` 付きの `send_input` は実行中ターンを中断させ、作業を失わせる。
- 停滞判定は経過時間ではなく worktree の変化で行う。間隔を空けた複数回の観察で log・status・diff に変化がなく `wait` も Running のままの場合に限り、interrupt なしの `send_input` を 1 回だけ送る。それでも次の `wait` とその後の観察で変化がない場合のみ `close_agent` + 再 spawn する。main agent が実装を引き継がない。
- 結果を読み終えた終了済み subagent は `close_agent` で閉じる（spawn スロットがリークし `AgentLimitReached` で spawn できなくなる既知問題の回避）。
