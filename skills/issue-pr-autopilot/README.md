...
- **終了状態は APPROVED / HANDOFF の 2 つ**: APPROVED = G5 の全条件を満たした状態（safety/migration/security の unchecked なし〔人間回答・分割除外・条件付き isolated_unverified で解消〕+ 仮承認後の最終 HEAD full validation 1 回 + PR description 同期 + required checks 成功）。HANDOFF = 予算ゲート発動または G5 未達での内部処理終了で、承認ではない。残指摘・cycle 表を PR に記録して人間に引き渡す
...
- **予算ゲート**: 構造条件（同一指摘 cluster の PARTIAL 判定 2 回 / inventory 外への拡張継続 / 新 layer・新規サブシステムの追加が必要）で修正ループを自動停止して HANDOFF に分岐する（security・データ破壊級の must-fix は default-off / feature flag / stage-out で隔離してから終了）。diff 2 倍 + 500 行超は scope-growth シグナル、wall-clock 120 分（external wait 除く）は SLO 通知で、どちらも単独では停止しない。内部レビューは 1 cycle 最大 2 round
