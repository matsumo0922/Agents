...
- **round 1 で出し尽くす + 有限 closure**: must-fix は意図アンカーと、同根クラスの全経路をまとめた ID 付き有限 inventory・「閉じる条件」を付けて初めて成立する。nit の既定処置は「対応不要（記録のみ）」。再レビューは inventory ID 単位の CLOSED / PARTIAL / NEW 判定のみで、同一指摘の要求を拡張しない。main は must-fix を「未対応だと issue のゴールが達成されないか」で今回必須 / follow-up / 過剰 / design defect の 4 分類に裁定する
