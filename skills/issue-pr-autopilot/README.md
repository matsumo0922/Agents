...
- **round 1 で出し尽くす + 有限 closure**: must-fix は意図アンカーと、同根クラスの全経路をまとめた ID 付き有限 inventory・「閉じる条件」を付けて初めて成立する。nit の既定処置は「対応不要（記録のみ）」。再レビューは inventory ID 単位の CLOSED / PARTIAL / NEW 判定のみで、同一指摘の要求を拡張しない。main は must-fix を意図アンカー（受け入れ条件・明示 invariant・今回導入した regression の暗黙の非退行 invariant）で今回必須 / follow-up / 過剰 / design defect の 4 分類に裁定する。アンカー済みの regression は「新機能のゴールは達成済み」を理由に follow-up へ降格できず、follow-up はアンカー不能な既存欠陥・スコープ外改善に限る
