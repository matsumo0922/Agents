...
- **受理ゲート G2〜G5**: G2 = 設計の gate schema 適合（必須セクション・条件付き matrix の発動・非該当宣言）、G3 = worker evidence matrix の受理（全行の具体性・production call path・SHA 一致。「全件修正済み」の要約だけの報告は不受理）、G4 = レビュー開始前の検証台帳 SHA と validation scope の照合、G5 = APPROVED 条件（safety/migration/security の unchecked なし〔人間回答による checked 再判定か分割除外だけが解消手段〕+ 最終 HEAD の full validation + PR description 同期 + required checks 成功）
- **設計ゲートと反証**: 設計が無い複数レイヤー変更は designer が設計契約準拠で設計し、客観条件リスト（cross-layer / safety / migration / security / 複数 consumer / DB hot path）に該当すれば clean context の falsifier が設計を反証してから実装に入る。該当判定は main が行う。blocking 反例（受け入れ条件・invariant・safety/migration/security を破るもの）は designer 単独の「受容」で閉じられず、設計修正 + falsifier 再確認か人間判断まで worker を spawn しない。高リスクな未検証前提は 3 分岐プロトコル（安全性・不可逆性を左右する未確定は停止 / reversible な stage のみ進行 / 既存状態維持のみ fail-safe 仮決め）で扱う
...
- **GitHub 投稿の最小化と妥当性判断の先行**: reviewer は GitHub に投稿せず review_result を main に返し、main が全指摘の妥当性を判断してから、妥当な指摘だけを round 1 レビューコメントとして投稿する。PR に投稿するのは round 1 レビューと最終 APPROVED コメント（サマリー + 指摘対応表 + cycle 表）のみ。PR description の更新も main の責務
