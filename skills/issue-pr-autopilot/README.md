...
- **受理ゲート G2〜G5**: G2 = 設計の gate schema 適合（必須セクション・条件付き matrix の発動・非該当宣言）、G3 = worker evidence matrix の受理（全行の具体性・production call path・SHA 一致。「全件修正済み」の要約だけの報告は不受理）、G4 = レビュー開始前の検証台帳 SHA と validation scope の照合、G5 = 確定条件（safety/migration/security の unchecked なし〔人間回答・分割除外・default-off 隔離のいずれかで解消〕+ 仮承認後の最終 HEAD full validation 1 回 + PR description 同期 + required checks 成功）
- **予算ゲート**: 同一指摘 cluster の PARTIAL 判定 2 回 / round 1 レビューから 120 分 / diff が round 1 時点の 2 倍 / 新 layer・新規サブシステムの追加が必要、のいずれかで修正ループを自動停止し、未解消の指摘を「人間レビューへの引き継ぎ」として PR に記録して確定に進む（security・データ破壊級の must-fix は default-off / feature flag / stage-out で隔離してから確定）。内部レビューは 1 cycle 最大 2 round
...
- **検証 tier と信頼チェーン**: 検証結果は「コマンド / 結果 / HEAD SHA / validation scope」の台帳で共有し、reviewer はテストを再実行しない。初回 = full、レビュー修正 = compile + targeted、rebase = affected、仮承認後の最終 HEAD = full 1 回（G5 必須）。heavy validation はマシン全体で lease 1 本に直列化し、worktree ごとに `GRADLE_USER_HOME` を分離する。環境依存で決定的にできない挙動は targeted テスト 1 本 + deploy 後確認（default-off）で証明できる
...
- **round 1 で出し尽くす + 有限 closure**: must-fix は同根クラスの全経路を 1 グループにまとめ、確認済み対象に ID を振った有限の inventory と「閉じる条件」を付ける。再レビューは inventory ID 単位の CLOSED / PARTIAL / NEW 判定のみで、同一指摘の要求を拡張しない。「round 1 網羅漏れ」の新規指摘は round 2 までで打ち切る
- **cycle / round の分離と計測**: 外部レビューは常に、CI failure・rebase は material な場合のみ新 cycle とし、round は cycle 内で数え直す（routine な再レビューを新 cycle に誤分類しない）。round ごとに finding origin・regression 件数・発動した予算ゲート・時間 4 区分（wall-clock / cumulative compute / critical-path / external wait）を即時記録し、最終コメントへ cycle 表として転記する
