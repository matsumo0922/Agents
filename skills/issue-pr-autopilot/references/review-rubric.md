...
- **意図アンカー**: 各指摘に「issue の受け入れ条件・設計 invariant のどれを守るための指摘か」の紐付けを付ける。must-fix はアンカー + 閉じる条件が揃って初めて成立する。**今回の diff が導入・悪化させた correctness / security / safety / 互換性の regression は、暗黙の非退行 invariant（変更が既存の正しい挙動を壊さないこと）にアンカーできる**（falsifier-rubric の scope 軸 introduced / worsened に対応する）。アンカーを付けられない指摘（変更対象外に既存の欠陥、スコープ外の改善）は内容の正しさに関わらず must-fix にせず、follow-up 提案として分類して報告する。
