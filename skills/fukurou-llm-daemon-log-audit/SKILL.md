...
description: "fukurou production の LLM daemon / paper trading runtime を read-only で確認し、llm_runs・command_event_log・decisions・paper ledger を時系列表に復元する。Use when the user asks to report current or recent fukurou LLM daemon behavior, paper trading status, fail-closed / NO_TRADE semantics, daemon chronology, scheduler skip reasons, or asks 翌日になったので続きを表にして."
---

# Fukurou LLM Daemon Log Audit

fukurou production の LLM daemon と paper trading 状態を、API と PostgreSQL の read-only query だけで確認する。実装変更、DB write、deploy、container restart は行わない。
...
- fukurou repo では shell command に `rtk` prefix を使う。remote shell に入る場合も、ローカル側の command は `rtk` で始める。
- paper trading / crypto bot の結果は投資助言として書かない。観測値、判断理由、ledger 事実に分けて説明する。

## Quick Workflow

1. fukurou repo で `AGENTS.md` と `~/.codex/RTK.md` を尊重する。
...
3. 時系列が必要なら、この skill の script を使う。

```bash
skill_script="${CODEX_HOME:-$HOME/.codex}/skills/fukurou-llm-daemon-log-audit/scripts/query-fukurou-llm-daemon-log.sh"
rtk proxy "${skill_script}" \
...
4. script output の `RUN|...` を時系列表へ変換する。`SKIP|...` は daemon が起動しなかった判断として別にまとめる。
...
- `reason` に「高値追い」「RSI過熱」「10,230,000抵抗」「押し目反発未確認」が並ぶ場合は、「上昇トレンドは認めつつ、入口品質が悪く見送り」と要約する。
