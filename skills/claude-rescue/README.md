...
既定値は `--model claude-opus-4-8` / `--effort high` / `--allowed-tools "Read,Grep,Glob,Bash"` で、引数で上書きできます。呼び出しは常に `--permission-mode dontAsk`（headless 向けの fail-closed）。Bash は既定で allow され、`rm -rf` / `sudo` などの危険コマンドは `~/.claude/settings.json` の deny が全モードで最優先に弾きます（Edit / Write は allow されません）。詳細な使い方と環境知識（Codex では `require_escalated` が必要、など）は [SKILL.md](SKILL.md) を参照してください。
